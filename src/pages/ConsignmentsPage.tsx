...
export default function ConsignmentsPage() {
  ...
  const addItem = (product: Product) => {
    ...
    setItems(prev => [...prev, {
      ...
      quantity: 1,
      consignorPrice: Number(product.cost_price) || 0,
    }]);
    setProductSearch("");
    ...
    <DashboardLayout>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-app-text-muted mt-1">
                  Se revertirá el stock ingresado. Esta acción no se puede deshacer.
                </p>
              </div>
            </div>
      ...
      <div className="mb-6">
        ...
        <p className="text-app-text-muted mt-1 text-sm">
          Registra mercancía recibida en consignación — el stock se actualiza sin generar una compra.
        </p>
      </div>
          ...
          <div className="p-6 flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Nombre del consignador *"
                  value={consignorName}
                ...
                {showDropdown && productSearch && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-app-bg border border-app-border rounded-xl shadow-2xl z-20 overflow-hidden">
                    ...
                    <button
                      ...
                      className="w-full flex items-center gap-2 px-4 py-2.5 border-t border-app-border text-app-text-muted hover:text-app-text text-xs transition-colors"
                    >
                      <X size={13} /> Cerrar
                    </button>
                  </div>
                ...
                {items.map((item, idx) => (
                  <div key={idx} className="grid grid-cols-12 gap-2 items-center bg-app-bg rounded-xl px-3 py-2 border border-app-border">
                    ...
                    <div className="col-span-3">
                      <input
                        type="number" min="0.001" step={item.unit_type === "WEIGHT" ? "0.001" : "1"}
                        value={item.quantity}
                    ...
                    <div className="col-span-4">
                      <input
                        ...
                        value={item.consignorPrice}
                        onChange={e => updateItem(idx, "consignorPrice", parseFloat(e.target.value) || 0)}
                ...
                <div key={c.id}>
                  ...
                  {isOpen && (
                    <div className="px-6 pb-4 animate-in zoom-in-95 duration-200">
                      {c.notes && (
                        <p className="text-xs text-app-text-muted mb-3 italic">{c.notes}</p>
                      )}
