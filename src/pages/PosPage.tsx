...
export default function PosPage() {
  ...
  return (
    ...
    <DashboardLayout>
      ...
      {/* ── TICKET FULL WIDTH ── */}
      <div className="flex-1 flex flex-col min-h-0 bg-app-sidebar border border-app-border rounded-2xl overflow-hidden">

        {/* Header */}
        <div className="bg-app-accent/10 px-5 py-3 border-b border-app-border flex justify-between items-center shrink-0">
          <div className="flex items-center gap-3">
            ...
            {cart.length > 0 && <span className="px-2 py-0.5 bg-app-accent text-white text-[10px] font-black rounded-full">{cart.length}</span>}
          </div>
          <div className="flex items-center gap-1">
            <button onClick={() => { fetchPendingSales(); setShowPending(true); }}
              ...
              <Clock size={17} />
              {pendingSales.length > 0 && <span className="absolute -top-1 -right-1 w-4 h-4 bg-amber-400 text-black text-[9px] font-black rounded-full flex items-center justify-center">{pendingSales.length}</span>}
            </button>
            {cart.length > 0 && (
        ...
        <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-4">
          {cart.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-app-text-muted opacity-20 gap-4">
                ...
                return (
                  <div key={key} className="flex flex-col bg-app-card rounded-xl p-3 border border-app-border group/cart">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <button onClick={() => removeFromCart(key)} className="text-app-text-muted hover:text-rose-500 transition-colors shrink-0 ml-1">
                        <Trash2 size={14} />
                      </button>
                    </div>
                    <div className="flex items-center justify-between mt-auto gap-1">
                      <div className="flex-1 min-w-0">
                        {isPriceEditing === key ? (
                          <div className="flex items-center gap-1">
                            <span className="text-app-accent font-bold text-xs">$</span>
                            <input type="number" autoFocus
                              className="w-full bg-app-bg border border-app-accent rounded px-1 py-0.5 text-sm font-bold text-app-accent focus:outline-none"
                          ...
                          <button onClick={() => setIsPriceEditing(key)}
                            className="text-app-accent font-black text-sm hover:bg-app-accent/5 px-1 py-0.5 rounded transition-colors truncate w-full text-left" title="Click para editar">
                            ${Number(item.customPrice).toLocaleString()}
                          </button>
                        )}
                      </div>
                      {item.product.unit_type === "WEIGHT" ? (
                        <span className="text-[10px] font-black text-amber-500 px-1.5 py-0.5 bg-amber-500/10 rounded border border-amber-500/20 uppercase shrink-0">{item.quantity}Kg</span>
                      ) : (
                        <div className="flex items-center bg-app-accent/10 rounded-lg p-0.5 border border-app-accent/10 shrink-0">
                          <button onClick={() => updateQuantity(key, -1)} className="p-0.5 hover:bg-app-accent/20 rounded text-app-accent transition-colors"><Minus size={12} /></button>
                          <span className="w-6 text-center text-xs font-black text-app-text">{item.quantity}</span>
                          <button onClick={() => updateQuantity(key, 1)} className="p-0.5 hover:bg-app-accent/20 rounded text-app-accent transition-colors"><Plus size={12} /></button>
                        </div>
                      )}
                    </div>
                  </div>
        ...
        {/* Checkout — fila horizontal */}
        <div className="bg-app-accent/5 border-t border-app-border px-5 py-4 shrink-0">
          <div className="flex flex-wrap items-end gap-3">

            {/* Método de pago */}
            <div className="flex bg-app-bg p-1 rounded-xl border border-app-border shadow-inner gap-1 shrink-0">
              {([
            ...
            {user?.saleTypeEnabled && (
              <div className="flex gap-1 bg-app-bg p-1 rounded-xl border border-app-border shrink-0">
                <button onClick={() => setSaleType("WHOLESALE")} className={`px-3 py-2 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all ${saleType === "WHOLESALE" ? "bg-violet-600 text-white shadow" : "text-app-text-muted hover:text-app-text"}`}>Mayor.</button>
                <button onClick={() => setSaleType("RETAIL")} className={`px-3 py-2 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all ${saleType === "RETAIL" ? "bg-app-accent text-white shadow" : "text-app-text-muted hover:text-app-text"}`}>Detal</button>
              </div>
            )}

            {/* Cliente (crédito) */}
            {paymentMethod === 'CREDIT' && (
              <div className="relative flex-1 min-w-48 animate-in slide-in-from-left-2 duration-200">
                {selectedCustomer ? (
                  <div className="flex items-center justify-between bg-app-accent/10 border border-app-accent/30 rounded-xl px-3 py-2">
                  ...
                  <div>
                    <div className="relative">
                      ...
                      <input ref={customerSearchRef} type="text" placeholder="Buscar cliente..."
                        ...
                        className="w-full bg-app-bg border border-app-border rounded-xl pl-9 pr-4 py-2 text-app-text placeholder-app-text-muted focus:outline-none focus:border-app-accent/50 text-sm"
                      />
                    </div>
              ...
              <div className="flex items-end gap-3 shrink-0">
                {(isTouch || isBillCompany) ? (
                  <button onClick={() => setShowCashPad(true)}
                    className="px-4 py-2 rounded-xl border-2 border-app-accent/40 bg-app-accent/10 text-app-accent font-black text-sm active:scale-95 transition-all flex items-center gap-2">
                    💵 {cashReceived ? `$${parseFloat(cashReceived).toLocaleString()} · Cambio: $${change.toLocaleString()}` : "Ingresar efectivo"}
                  </button>
                ) : (
                  <>
                    <div>
                      <div className="text-[9px] font-black text-app-text-muted uppercase tracking-widest mb-1">Recibido</div>
                      <div className="relative">
                    ...
                    <div className="bg-emerald-500/10 px-3 py-1.5 rounded-xl border border-emerald-500/20 mb-0.5">
                      <div className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">Cambio</div>
                      <div className="text-lg font-black text-emerald-500">${change.toLocaleString()}</div>
                    </div>
                  </>
                )}
              </div>
            ...
            <div className="flex items-center gap-4 ml-auto shrink-0">
              <div className="text-right">
                <div className="text-[10px] font-black text-app-text-muted uppercase tracking-widest">A pagar</div>
