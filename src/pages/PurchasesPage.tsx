...
interface PurchaseItem { productId: string; productName: string; sku: string; unit_type: string; quantity: number; cost: number; }
    ...
    return (
        <DashboardLayout>
            <div className="mb-6">
                ...
                <p className="text-white/40 mt-1 text-sm">Registra recepciones de mercancía — el stock se actualiza automáticamente.</p>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl backdrop-blur-md shadow-xl overflow-hidden">
                    <div className="px-6 py-4 border-b border-white/10 flex items-center gap-2">
                        ...
                        <h2 className="font-bold text-white">Nueva Recepción de Mercancía</h2>
                    </div>

                    <div className="p-6 flex flex-col gap-5">
                        {/* Proveedor */}
                        <div>
                            <label className="block text-xs font-medium text-white/50 mb-1.5">Proveedor (opcional)</label>
                            <div className="flex gap-2">
                                <select
                                    value={selectedSupplier}
                                    onChange={e => setSelectedSupplier(e.target.value)}
                                    className="flex-1 bg-black/30 border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40"
                                >
                            ...
                            {showNewSupplier && (
                                <div className="mt-3 p-4 bg-black/20 rounded-xl border border-white/5 flex flex-col gap-3">
                                    <input
                                        type="text"
                                        placeholder="Nombre del proveedor *"
                                        value={newSupplierName}
                                        onChange={e => setNewSupplierName(e.target.value)}
                                        className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40"
                                    />
                                    <input
                                        type="text"
                                    ...
                                    />
                                    <div className="flex gap-2">
                                        <button onClick={() => setShowNewSupplier(false)} className="flex-1 py-2 rounded-lg border border-white/10 text-white/50 text-sm hover:text-white transition-colors">Cancelar</button>
                        ...
                        <div>
                            <label className="block text-xs font-medium text-white/50 mb-1.5">Agregar Productos</label>
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full bg-black/30 border border-white/10 rounded-xl pl-9 pr-4 py-2.5 text-white text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/40"
                                />
                                {showProductDropdown && filteredProducts.length > 0 && (
                                    <div className="absolute top-full left-0 right-0 mt-1 bg-[#0f172a] border border-white/10 rounded-xl shadow-2xl z-20 overflow-hidden">
                                        {filteredProducts.map(p => (
                                            ...
                                            >
                                                ...
                                                <span className="text-white/30 text-xs">{p.unit_type === "WEIGHT" ? "Kg/Lts" : "Unid."}</span>
                                            </button>
                                        ))}
                                    </div>
                                )}
                        ...
                        {items.length > 0 && (
                            <div className="flex flex-col gap-2">
                                <div className="grid grid-cols-12 gap-2 px-1 text-[10px] font-bold text-white/30 uppercase">
                                ...
                                {items.map(item => (
                                    <div key={item.productId} className="grid grid-cols-12 gap-2 items-center bg-black/20 rounded-xl px-3 py-2 border border-white/5">
                                        <div className="col-span-5">
                                            <p className="text-sm font-medium text-white/80 truncate">{item.productName}</p>
                                            <p className="text-[10px] text-white/30">{item.sku} · {item.unit_type === "WEIGHT" ? "Kg" : "Unid."}</p>
                                        </div>
                                        <div className="col-span-2">
                                            <input
                                                ...
                                                className="w-full bg-black/40 border border-white/10 rounded-lg px-2 py-1.5 text-white text-sm text-center focus:outline-none focus:ring-1 focus:ring-violet-500/40"
                                            />
                                        </div>
                                        ...
                                        <div className="col-span-1 text-right">
                                            <p className="text-emerald-400 font-bold text-xs">{cop(item.quantity * item.cost)}</p>
                                        </div>
                                        <div className="col-span-1 flex justify-end">
                                            <button onClick={() => removeItem(item.productId)} className="text-white/20 hover:text-rose-400 transition-colors p-1">
                                                <Trash2 size={14} />
                                            </button>
                                        </div>
                                    </div>
                                ...
                                {/* Total + Submit */}
                                <div className="flex flex-col gap-4 px-3 py-4 bg-black/40 rounded-2xl border border-white/5 mt-4">
                                    <h4 className="text-[10px] font-bold text-violet-400 uppercase tracking-widest">Información de Pago</h4>
                                    
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-[10px] text-white/40 mb-1">Monto Pagado Inicial</label>
                                            <input 
                                                type="number" 
                                                placeholder={total.toString()}
                                                value={paidAmount}
                                                onChange={e => setPaidAmount(e.target.value)}
                                                className="w-full bg-black/30 border border-white/10 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:ring-1 focus:ring-violet-500/50"
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-[10px] text-white/40 mb-1">Método de Pago</label>
                                            <select 
                                                value={paymentMethod}
                                    ...
                                    {(paidAmount && parseFloat(paidAmount) < total) && (
                                        <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                                            <label className="block text-[10px] text-white/40 mb-1">Fecha Límite de Pago (Opcional)</label>
                                            <input 
                                                type="date" 
                                                value={dueDate}
                                    ...
                                    <div className="flex items-center justify-between border-t border-white/5 pt-4">
                                        <span className="text-white/50 font-medium text-sm">Restante / Deuda</span>
                                        <span className={`text-xl font-black ${parseFloat(paidAmount || "0") < total ? "text-rose-400" : "text-white/20"}`}>
                                            {cop(Math.max(0, total - parseFloat(paidAmount || "0")))}
                                        </span>
                                    </div>
                                </div>
                                <div className="flex items-center justify-between px-3 py-3 mt-1">
                                    <span className="text-white/50 font-medium text-sm">Total de la Compra</span>
                                    <span className="text-2xl font-black text-white">{cop(total)}</span>
                                </div>
                                <button
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl backdrop-blur-md shadow-xl overflow-hidden flex flex-col">
                    <div className="px-1 py-1 border-b border-white/10 flex bg-black/20">
                        <button 
                            onClick={() => setView("history")}
                            className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-bold transition-all rounded-xl ${view === "history" ? "bg-white/10 text-cyan-400 shadow-lg" : "text-white/30 hover:text-white"}`}
                        >
                            <Calendar size={16} /> Historial
                        </button>
                        <button 
                            onClick={() => setView("debts")}
                            className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-bold transition-all rounded-xl ${view === "debts" ? "bg-rose-500/10 text-rose-400 shadow-lg" : "text-white/30 hover:text-white"}`}
                        >
                    ...
                    {view === "history" && (
                        <div className="px-6 py-4 border-b border-white/10 animate-in fade-in duration-300">
                            <h2 className="font-bold text-white flex items-center gap-2 mb-3 text-xs uppercase tracking-widest opacity-50">
                                Filtros de Historial
                            </h2>
                            <div className="flex gap-2">
                                <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)}
                                    className="flex-1 bg-black/30 border border-white/10 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/40" />
                    ...
                    {view === "debts" && (
                        <div className="px-6 py-4 border-b border-white/10 bg-rose-500/5 animate-in fade-in duration-300">
                            <div className="flex justify-between items-center">
                                <div>
                                    <h2 className="font-bold text-rose-400 text-xs uppercase tracking-widest">Resumen de Deuda</h2>
                                    <p className="text-[10px] text-white/30">Facturas pendientes con proveedores</p>
                                </div>
                    ...
                    <div className="flex-1 overflow-y-auto divide-y divide-white/5">
                        {isLoadingHistory ? (
                            <div className="flex justify-center items-center py-16 gap-2 text-white/30">
                                ...
                                <div key={p.id} className={view === "debts" ? "border-l-4 border-rose-500/30" : ""}>
                                        ...
                                        <div className="text-right shrink-0">
                                            {view === "history" ? (
                                                ...
                                                <div className="flex flex-col items-end">
                                                    ...
                                                    <span className="text-[10px] text-white/20 line-through">{cop(Number(p.total))}</span>
                                                </div>
                                            )}
                                        </div>
                                    ...
                                    {isOpen && (
                                        <div className="px-6 pb-4 animate-in zoom-in-95 duration-200">
                                            <div className="bg-black/20 rounded-xl overflow-hidden border border-white/5 mb-3">
                                                <div className="grid grid-cols-12 gap-2 px-4 py-2 text-[10px] font-bold text-white/30 uppercase border-b border-white/5">
                                                    <span className="col-span-12">Detalles de Deuda</span>
                                                </div>
                                                <div className="p-4 grid grid-cols-2 gap-4">
                                                    <div className="space-y-1">
                                                        <p className="text-[10px] text-white/30 uppercase font-bold">Estado de Pago</p>
                                                    ...
                                                    <div className="space-y-1">
                                                        <p className="text-[10px] text-white/30 uppercase font-bold">Fecha de Vencimiento</p>
                                                        <p className={`text-xs font-bold ${isOverdue ? "text-rose-400" : "text-white/60"}`}>
                                            ...
                                            <div className="bg-black/20 rounded-xl overflow-hidden border border-white/5">
                                                ...
                                                {p.purchase_items?.map((item, idx) => (
                                                    <div key={idx} className="grid grid-cols-12 gap-2 px-4 py-2.5 border-b border-white/5 last:border-0">
                                                        <div className="col-span-5">
                                                            <p className="text-sm text-white/70">{item.products?.name || 'N/A'}</p>
                                                            <p className="text-[10px] text-white/30">{item.products?.sku || '---'}</p>
                                                        </div>
                                                        <span className="col-span-3 text-right text-sm text-white/60">
