...
const ZXING_CDN = "https://cdn.jsdelivr.net/npm/@zxing/library@0.22.0/umd/index.min.js";
...
export default function BarcodeScannerModal({ onScan, onClose }: Props) {
    ...
    const startScanner = async (deviceId?: string) => {
        ...
        try {
            ...
            const devices: MediaDeviceInfo[] = await ZXing.BrowserMultiFormatReader.listVideoInputDevices();
            setCameras(devices);

            let camId = deviceId;
            if (!camId) {
                const back = devices.find((d: MediaDeviceInfo) => /back|rear|environment/i.test(d.label));
                camId = back?.deviceId ?? devices[0]?.deviceId;
            ...
            await reader.decodeFromVideoDevice(camId, videoRef.current, (result: any, err: any) => {
                if (result) {
                    stopReader();
            ...
            if (e?.name === "NotAllowedError") {
                ...
                setError("No se pudo iniciar el escáner. Usa el campo manual.");
            }
        }
    };
            ...
            <div className="relative w-full max-w-sm bg-app-bg border border-app-border rounded-2xl shadow-2xl overflow-hidden">
                ...
                <div className="relative bg-black" style={{ aspectRatio: "4/3" }}>
                    ...
                    {error && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-black/80 p-6">
                            ...
                            <button onClick={() => startScanner(selectedCamera || undefined)}
                ...
                <div className="px-4 py-3 flex flex-col gap-2">
                    <p className="text-[11px] text-app-text-muted text-center">
                        Apunta la cámara al código · o ingrésalo manualmente
                    </p>
                    <div className="flex gap-2">
                        ...
                        <button onClick={handleManual}
                            className="px-3 py-2 bg-app-accent rounded-xl text-white font-bold text-sm">
                            <ScanLine size={15} />
                        </button>
                    </div>
                </div>
