...
export default function StatCard({ title, value, subtitle, icon, colorFrom, colorTo }: StatCardProps) {
    return (
        <div className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${colorFrom} ${colorTo} p-6 shadow-xl border border-white/10 group transition-transform hover:-translate-y-1 duration-300`}>
            {/* Glass decoration */}
            ...
            <div className="relative z-10 flex flex-col h-full">
                <div className="flex justify-between items-start mb-4">
                    <h3 className="text-white/80 font-medium text-sm drop-shadow-sm">{title}</h3>
                    {icon && (
                ...
                <div className="mt-auto">
                    ...
                    {subtitle && (
                        <p className="text-xs text-white/70 mt-2 font-medium">{subtitle}</p>
                    )}
                </div>
            </div>
        </div>
